...
def run(structure: str, output: str, fmax: float, steps: int, kmesh: tuple[int,int,int], basis: str) -> None:
    out = Path(output)
    out.mkdir(parents=True, exist_ok=True)
    with Path(structure).open('r', encoding='utf-8') as handle:
        atoms = read_vasp(handle)
    atoms.calc = GPAW(mode='lcao', basis=basis, xc='PBE', kpts=kmesh,
...
def main() -> None:
    ...
    p.add_argument('--basis', default='dzp')
    args = p.parse_args()
    run(args.structure, args.output, args.fmax, args.steps, tuple(args.kmesh), args.basis)
